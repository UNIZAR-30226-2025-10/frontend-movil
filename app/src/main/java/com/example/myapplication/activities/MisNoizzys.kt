                            ...
                            override fun onResponse(call: Call<SearchPlaylistResponse>, response: Response<SearchPlaylistResponse>) {
                                if (response.isSuccessful) {
                                    ...
                                    if (response.code() == 401 && !yaRedirigidoAlLogin) {
                                        ...
                                        val intent = Intent(this@MisNoizzys, Inicio::class.java)
                                        startActivity(intent)
                                        finish()
                                        Toast.makeText(this@MisNoizzys, "Sesión iniciada en otro dispositivo", Toast.LENGTH_SHORT).show()
                                    }
                                }
                            }
            ...
            override fun onResponse(call: Call<AudioResponse>, response: Response<AudioResponse>) {
                if (response.isSuccessful) {
                    response.body()?.let { audioResponse ->
                        ...
                        ReproduccionTracker.startTracking(this@MisNoizzys, id) {
            ...
            override fun onResponse(call: Call<AudioResponse>, response: Response<AudioResponse>) {
                if (response.isSuccessful) {
                    response.body()?.let { audioResponse ->
                        ...
                        ReproduccionTracker.startTracking(this@MisNoizzys, ordenColeccion[indice]) {
                            notificarReproduccion()
                        }
